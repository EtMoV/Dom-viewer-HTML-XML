...
function visualize() {
    ...
    function hideZones() {
        document.getElementById("structureArborescente").innerHTML = "<h2>Structure arborescente : </h2>";
        document.getElementById("zoneAffichage").innerHTML = "<h2 class='titreZoneAfichage'>Zone d'affichage : </h2>";
        document.getElementById("infoElement").innerHTML = '<h2>Informations Balise :</h2><ul><li id="infoInnerHtml" class="list-group-item"></li><li id="infoTypeBalise" class="list-group-item"></li><li id="infoId" class="list-group-item"></li><li id="infoClass" class="list-group-item"></li><li id="infoTags" class="list-group-item"></li><li id="infoEnfants" class="list-group-item"></li></ul>';
    }
    hideZones();

    /* Permet d'afficher les barre horizontale*/
    function hrDisplay() {
        for (var i = 0; i < document.getElementsByClassName("separationV").length; ++i) {
            document.getElementsByClassName("separationV")[i].style.display = "block";
        }
        for (var i = 0; i < document.getElementsByClassName("separationH").length; ++i) {
    ...
    if (document.getElementById("urlInput").value) {
        ...
        if (xhr.status === 200) {
            ...
            document.getElementById("structureArborescente").innerHTML += addXMP(xhr.response);
            ...
            document.getElementById("errorAjax").innerHTML = "<span class='warn'>Il y a eu un problème lors de la requête AJAX</span>";
        ...
        reader.addEventListener('load', function () { // document.querySelector('#uploadInput').files[0].name + '" :\n\n' +
            document.getElementById("structureArborescente").innerHTML += addXMP(reader.result);
            ...
            */
            function getInfoElement(childrens) {
                for (var child of childrens) {
                    if (child.classList.contains("titreZoneAfichage")) { //il ne faut pas ajouter d'evenement au titre
                    ...
                    child.addEventListener('mousedown', function (e) {
                        ...
                        currentBalise = this;
                        document.getElementById('infoInnerHtml').innerHTML = "InnerHtml de la balise : " + this.innerHTML;
                        document.getElementById('infoTypeBalise').innerHTML = "Type de balise : " + this.tagName;
                        document.getElementById('infoId').innerHTML = "Id : " + this.id;
                        document.getElementById('infoClass').innerHTML = "Class : " + this.classList;
                        DisplayChilds(this);
                        DisplayTags(this);
                    });
                }
        ...
        alert("Vous devez uploadez ou renseignez un fichier au format HTML ou XML !");
        document.getElementById("uploadInput").value = '';
        document.getElementById("urlInput").value = '';
    }
}
...
function enregistrerDocument() {
    ...
    function getDocOnJSON() {
        var element = Object.assign(document.getElementById('zoneAffichage'));
        ...
        var debut = '{"html":"';
        var fin = '"}';
        var jsonReformate = debut + jsonReformate + fin
        return jsonReformate;
    }
    getDocOnJSON();
...
function recupererDocument() {
    ...
    if (id) {
        ...
        if (req.status === 200) {
            ...
            console.log("Réponse reçue: \n-------------\n", req.response, "\n-------------");
            document.getElementById("structureArborescente").innerHTML += addXMP(data.html);
            document.getElementById("zoneAffichage").innerHTML += data.html;
        } else {
            console.log("Status de la réponse: %d (%s)", req.status, req.statusText);
            document.getElementById("errorAjax").innerHTML = "<span class='warn'>Il y a eu un problème lors de la requête AJAX</span>";
        }
    } else {
        document.getElementById("errorAjax").innerHTML = "<span class='warn'>Vous n'avez pas indiqué d'identifiant à récupéré</span>";
    }
}
